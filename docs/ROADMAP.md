...
**Root-cause direction:** L0 is `compress_ratio=0` (non-compressed sliding-window attention). The divergence is between `prefill_start_no_compress_from_device` (batched, true multi-token attention with causal masking) and `decode_step_no_compress_from_device` (single-token, incremental KV append). For a causal model these should produce identical last-token outputs. Initial code inspection found a concrete decode-side mismatch: CUDA `linear_matvec_from_device` applies activation quantization **in-place** to its input buffer, but the no-compress and compressed decode paths reused the same `hidden_device` for both `query_a` and `key_value`. That means token-loop decode projected KV from a hidden vector already quantized for `query_a`, while batched prefill's rows path clones/prepares row inputs independently before each linear. `crates/ferrule-model/src/models/deepseek_v4/attention.rs` now clones a separate `kv_hidden_device` before `query_a` in the affected CUDA decode paths and uses it for the KV projection. Graph-safe replay already recopies `hidden_dev` into `arena.hidden_q` before KV projection, so it was not changed.

**Validation state:** this is a code-level fix candidate, not yet model-validated in this workspace because `models/` is empty. `cargo fmt` and `cargo check -p ferrule-model --features cuda` pass, aside from the pre-existing workspace `unsafe_code` warning in `moe/streaming.rs`. The required next validation is to rerun `deepseek-v4-prefill-parity -p Hello --chat --max-layers 43 --backend cuda` with the full model and confirm whether L0 `max_abs_diff` collapses to numerical noise. If it does not, continue with stage-level instrumentation (Qa/QNorm/Qb/QNormHeads/KV projection/RoPE/sparse attention/output projection) within L0.

**Open question:** the `[19923]` active blocker may be resolved, but the L0 batched-vs-token-loop divergence remains a real correctness risk until the hidden-clone fix is measured. Even though top-1 currently matches, the hidden-state gap (up to 19 at L42 in the stale harness run) means any model weight or routing change could flip the top-1. The divergence should be fixed or explicitly explained before relying on batched prefill for production.
...
Eager 43L prefill now produces `[30594]` ("你好"), matching the last known-good result. The `[19923]` regression is no longer reproducible. The remaining correctness risk is the L0 batched-vs-token-loop divergence (4.7e-2 at L0, growing to 19 at L42 in the last recorded harness run); see the prefill parity harness results above. A decode-side hidden-buffer aliasing fix has landed in `attention.rs`, but the full-model parity harness still needs to be rerun before treating this risk as closed. The next performance bottleneck is still **expert residency for first useful turn**, not generic scheduler overhead:
...
|---|---|---|
| **P0** | Revalidate L0 prefill parity after decode hidden-clone fix | Prefill parity harness showed batched vs token-loop diverging at L0 (4.7e-2) for multi-token prompts. Code inspection found token-loop CUDA decode reused a `hidden_device` mutated in-place by activation quantization for `query_a`, then fed that mutated buffer into `key_value`; the decode paths now use a separate `kv_hidden_device`. Rerun the full-model parity harness to confirm the L0 diff collapses. If not, fall back to stage-level (Qa/QNorm/Qb/KV/RoPE/attn/output) comparison to find the next first-diverging sub-stage. |
