...
fn fp8_prepacked_linear_matches_scalar_reference() {
    let _guard = cuda_test_guard();
    if !has_cuda() {
        eprintln!("skipping: no CUDA device");
        return;
    ...
    unsafe {
        module.gemm_fp8_e4m3fn_e8m0_prepacked(
            &stream,
            LaunchConfig {
                grid_dim: (1, 1, 1),
                block_dim: (32, 1, 1),
...
#[test]
fn grouped_output_a_bf16_from_fp8_matches_reference() {
    let _guard = cuda_test_guard();
    if !has_cuda() {
        eprintln!("skipping: no CUDA device");
        return;
    ...
    assert!(ops.grouped_output_a_prepacked_fp8_supported(&handle, OUT, GROUP_IN, RANK));
    ...
    ops.grouped_output_a_bf16_from_fp8_into(
        &context_dev,
        ROWS,
        &handle,
        OUT,
...
#[test]
fn fp8_prepacked_rows_match_matvec() {
    let _guard = cuda_test_guard();
    if !has_cuda() {
        eprintln!("skipping: no CUDA device");
        return;
    ...
    assert!(ops.artifact_linear_supports_prepacked_fp8(&handle));
    ...
    if native_kernel_available(CutlassKernelId::Fp8Projection) {
        ...
        ops.artifact_fp8_projection_rows_from_device_into_with_scratch(
            &handle,
            &input_dev,
            ROWS,
            &mut native_output,
