...
namespace ferrule::cuda::cutlass::test_oracles::hybrid_mla_attention::
    preserve_f32_order {
...
inline Status launch(const Args *args) {
  ...
  // ferrule::cuda::cutlass::test_oracles::hybrid_mla_attention::preserve_f32_order
...
namespace semantic = operators::hybrid_mla_attention::explicit_selection;
namespace oracle = preserve_f32_order::test_oracle;
...
inline semantic::ExplicitSelectionStatus
make_args(const semantic::ExplicitSelectionArgs *args,
          preserve_f32_order::Args *result) {
  if (args == nullptr || result == nullptr) {
    return semantic::ExplicitSelectionStatus::kInvalidArgument;
  }
  const auto contract_status =
      semantic::validate_explicit_selection_contract(args);
  if (contract_status != semantic::ExplicitSelectionStatus::kSuccess) {
    return contract_status;
  }
  semantic::workspace::Binding scratch{};
  const auto workspace_status = semantic::workspace::bind(args, &scratch);
  if (workspace_status != semantic::ExplicitSelectionStatus::kSuccess) {
    return workspace_status;
  }

  *result = preserve_f32_order::Args{
      args->kind,
      args->rows,
  ...
  return semantic::ExplicitSelectionStatus::kSuccess;
}

} // namespace detail

inline semantic::ExplicitSelectionStatus
scalar_launch(const semantic::ExplicitSelectionArgs *args) {
#if FERRULE_CUDA_HAS_BASELINE_SIMT
  preserve_f32_order::Args oracle_args{};
  const auto status = detail::make_args(args, &oracle_args);
  return status == semantic::ExplicitSelectionStatus::kSuccess
             ? oracle::launch(&oracle_args)
             : status;
#else
  (void)args;
  return semantic::ExplicitSelectionStatus::kUnsupported;
#endif
}

inline semantic::ExplicitSelectionStatus
compare_launch(const semantic::ExplicitSelectionArgs *args,
  ...
  const auto status = detail::make_args(args, &oracle_args);
  return status == semantic::ExplicitSelectionStatus::kSuccess
             ? oracle::launch_compare(&oracle_args, oracle_output_f32,
                                      compare_result_i32)
             : status;
#else
  (void)args;
  (void)oracle_output_f32;
  (void)compare_result_i32;
  return semantic::ExplicitSelectionStatus::kUnsupported;
#endif
}
...
#endif // FERRULE_CUDA_TEST_ORACLE
