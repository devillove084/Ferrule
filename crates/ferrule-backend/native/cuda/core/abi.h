...
typedef enum FerruleCoreRowsKind {
  FERRULE_CORE_ROWS_F32_TO_BF16_RNE = 1,
} FerruleCoreRowsKind;

/*
 * Provider-neutral strided row conversion. Source and destination describe the
 * same logical [rows, heads, dimensions] tensor. All capacities and strides are
 * bytes; the conversion writes IEEE BF16 storage words using round-to-nearest,
 * ties-to-even without staging through host memory.
 */
typedef struct FerruleCoreRowsArgs {
  uint32_t kind;
  uint32_t rows;
  uint32_t heads;
  uint32_t dimensions;
  ...
  uint64_t input_head_stride_bytes;
...
typedef enum FerruleCoreRopeKind {
  ...
  FERRULE_CORE_ROPE_SPLIT_HALF_INDEXED = 4,
} FerruleCoreRopeKind;
...
typedef struct FerruleCoreRopeArgs {
  ...
  uint32_t rope_dim;
  uint32_t pair_count;
  /* Split-half indexed kind uses this as the cosine/sine table row count. */
...
typedef enum FerruleCoreRouterKind {
  ...
  FERRULE_CORE_ROUTER_SELECTED_SOFTMAX_TOPK = 5,
...
typedef enum FerruleCoreTransformerKind {
...
/*
 * Provider-neutral packed-row transformer contract. All tensor strides and
 * capacities are bytes. K and V use independent allocations and independent
 * page strides. Row metadata is packed: each query row names its sequence,
 * absolute append/query position, and logical KV length. The combined kind
 * launches append before attention on the same stream, making every pending
 * row visible subject to the per-row causal position.
 */
typedef struct FerruleCoreTransformerArgs {
  uint32_t kind;
  uint32_t rows;
  uint32_t sequences;
  uint32_t q_heads;
  ...
  uint64_t query_row_stride_bytes;
  uint64_t query_head_stride_bytes;
  uint64_t append_key_bf16;
  uint64_t append_key_bytes;
  uint64_t append_key_row_stride_bytes;
  uint64_t append_key_head_stride_bytes;
  uint64_t append_value_bf16;
  uint64_t append_value_bytes;
  uint64_t append_value_row_stride_bytes;
  uint64_t append_value_head_stride_bytes;
  uint64_t key_cache_bf16;
  uint64_t key_cache_bytes;
  uint64_t key_slot_stride_bytes;
  uint64_t key_layer_stride_bytes;
  uint64_t key_token_stride_bytes;
  uint64_t key_head_stride_bytes;
  uint64_t value_cache_bf16;
  uint64_t value_cache_bytes;
  uint64_t value_slot_stride_bytes;
  uint64_t value_layer_stride_bytes;
  uint64_t value_token_stride_bytes;
  uint64_t value_head_stride_bytes;
  uint64_t block_slots_i32;
  uint64_t block_slots_count;
  uint64_t block_offsets_i32;
  uint64_t block_offsets_count;
  uint64_t row_sequence_ids_i32;
  uint64_t row_sequence_ids_count;
  uint64_t row_positions_i32;
  uint64_t row_positions_count;
  uint64_t row_kv_lens_i32;
  uint64_t row_kv_lens_count;
  uint64_t output_f32;
  uint64_t output_bytes;
  uint64_t output_row_stride_bytes;
  uint64_t output_head_stride_bytes;
...
FERRULE_CORE_ASSERT_LAYOUT(FerruleCoreRowsArgs, 88, kind, input_f32, 16, 80);
...
FERRULE_CORE_ASSERT_LAYOUT(FerruleCoreTransformerArgs, 376, kind, query_f32, 48,
                           368);
...
static_assert(offsetof(FerruleCoreTransformerArgs, key_cache_bf16) == 144,
              "FerruleCoreTransformerArgs key cache offset mismatch");
static_assert(offsetof(FerruleCoreTransformerArgs, value_cache_bf16) == 192,
              "FerruleCoreTransformerArgs value cache offset mismatch");
static_assert(offsetof(FerruleCoreTransformerArgs, output_f32) == 320,
              "FerruleCoreTransformerArgs output offset mismatch");
static_assert(offsetof(FerruleCoreTransformerArgs, status_i32) == 352,
              "FerruleCoreTransformerArgs status offset mismatch");
