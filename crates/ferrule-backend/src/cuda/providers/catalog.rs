...
use crate::cuda::providers::cutlass::{
    CutlassKernelId, CutlassProviderManifest, discover_provider, kernel_for_operation,
};

use crate::plan::{
    ExecutionMode, KernelId, KernelOperation, KernelProviderId, LaunchDescriptor, LayerKernelPlan,
    LayerKernelRequirements, LinearBundleRequirement, ModelKernelPlan, OperationRequirement,
    ProviderRegistry, WeightLayout,
};
...
impl CudaProviderCatalog {
    ...
    pub fn discover() -> Result<Self> {
        let cutlass_provider = discover_provider()?;
        let cutlass = cutlass_provider.manifest();
        let mut registry = ProviderRegistry::new();
        registry.register(cutlass_provider.execution_manifest()?);
...
fn require_semantic_operation(
    ...
    let kernel = kernel_for_operation(operation).ok_or_else(|| BackendError::Invariant {
        message: format!(
...
fn validate_linear_bundle(requirement: &LinearBundleRequirement) -> Result<()> {
