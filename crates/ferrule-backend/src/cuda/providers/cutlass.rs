...
use crate::plan::{
    ExecutionModeSet, KernelOperation, KernelProviderId, OperationCapability, ProviderManifest,
};
...
impl CutlassProviderManifest {
    ...
    pub(crate) fn execution_manifest(self) -> ProviderManifest {
        let operations = CUTLASS_OPERATION_BINDINGS
            .iter()
...
impl CutlassProvider {
    ...
    pub fn execution_manifest(self) -> Result<ProviderManifest> {
        Ok(self.manifest.execution_manifest())
    }
}

pub(crate) const CUTLASS_OPERATION_BINDINGS: &[(KernelOperation, CutlassKernelId)] = &[
...
pub(crate) fn kernel_for_operation(operation: KernelOperation) -> Option<CutlassKernelId> {
    CUTLASS_OPERATION_BINDINGS
        .iter()
        .find_map(|&(candidate, kernel)| (candidate == operation).then_some(kernel))
}
...
#[repr(C)]
pub(crate) struct CutlassBf16CompressorArgs {
    rows: u32,
    n1: u32,
    n2: u32,
    k: u32,
...
#[derive(Debug, Clone, Copy)]
#[repr(C)]
pub(crate) struct CutlassFp8QueryAKvArgs {
    rows: u32,
    n1: u32,
    n2: u32,
    k: u32,
...
#[derive(Debug, Clone, Copy)]
#[repr(C)]
pub(crate) struct CutlassMainProjectNormArgs {
    rows: u32,
    input_size: u32,
    output_size: u32,
    scale_cols: u32,
...
#[derive(Debug, Clone, Copy)]
#[repr(C)]
pub(crate) struct CutlassHybridMlaAttentionArgs {
    block_rows: u32,
    heads: u32,
    head_dim: u32,
    sequence_tokens: u32,
...
#[derive(Debug, Clone, Copy)]
#[repr(C)]
pub(crate) struct FerruleCutlassHybridMlaExplicitSelectionArgs {
    kind: u32,
    rows: u32,
...
#[derive(Debug, Clone, Copy)]
#[repr(C)]
pub(crate) struct CutlassProposalHeadArgs {
    rows: u32,
    hc: u32,
    hidden: u32,
    vocab: u32,
...
#[derive(Debug, Clone, Copy)]
#[repr(C)]
pub(crate) struct CutlassHcProducerArgs {
    rows: u32,
    hc: u32,
    hidden: u32,
    mix: u32,
...
#[derive(Debug, Clone, Copy)]
#[repr(C)]
pub(crate) struct CutlassSharedFfnArgs {
    input_fp8: u64,
    input_ue8m0: u64,
    gate_weight_fp8: u64,
...
#[derive(Debug, Clone, Copy)]
#[repr(C)]
pub(crate) struct CutlassMlaOutputArgs {
    rows: u32,
    context_size: u32,
    groups: u32,
...
#[derive(Debug, Clone, Copy)]
#[repr(C)]
pub(crate) struct FerruleCutlassGroupedFp4MoeArgs {
...
#[derive(Debug, Clone, Copy)]
#[repr(C)]
pub(crate) struct PrepareMxfp4SfbArgs {
    n: u32,
    k: u32,
    reserved0: u32,
    linear_source: u64,
...
pub fn hybrid_mla_explicit_selection_workspace_requirements(
    ...
    let mut requirements = CutlassWorkspaceRequirements {
        bytes: 0,
        alignment: 0,
        reserved: 0,
    };
...
pub fn mla_output(
    ...
    let args = CutlassMlaOutputArgs {
        ...
        latent_bf16: latent.cu_deviceptr(),
