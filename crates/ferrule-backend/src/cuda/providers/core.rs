...
/// Handle to the statically linked Core CUDA operator set.
#[derive(Debug)]
pub struct CoreOperators {
    _context: Arc<CudaContext>,
}

pub fn load(context: &Arc<CudaContext>) -> KernelResult<CoreOperators> {
    context
        .bind_to_thread()
...
impl CoreOperators {
    ...
    pub unsafe fn f32_to_bf16_rne_rows(
        ...
        dimensions: u32,
        input_bytes: u64,
        input_row_stride_bytes: u64,
        input_head_stride_bytes: u64,
        output_bytes: u64,
        output_row_stride_bytes: u64,
        output_head_stride_bytes: u64,
    ) -> KernelResult<()> {
        let mut args = RowsArgs {
            kind: ROWS_F32_TO_BF16_RNE,
            rows,
            heads,
            dimensions,
            input_f32: device_ptr(input),
            input_bytes,
            input_row_stride_bytes,
            input_head_stride_bytes,
            output_bf16: device_ptr(output),
            output_bytes,
            output_row_stride_bytes,
            output_head_stride_bytes,
            ..Default::default()
        };
        invoke!(
            stream,
            args,
            ferrule_core_rows_launch,
            "F32 to BF16 RNE rows"
        )
    }
    ...
    pub unsafe fn rope_split_half_rows_indexed(
        ...
        let mut args = RopeArgs {
            kind: ROPE_SPLIT_HALF_INDEXED,
            rows,
            heads,
            head_dim,
            rope_dim,
            pair_count,
            start_position: table_positions,
            inverse,
            restore_bf16_boundary,
            values: device_ptr(values),
            cosine: device_ptr(cos_table),
            sine: device_ptr(sin_table),
            positions: device_ptr(positions),
            ..Default::default()
    ...
    pub unsafe fn selected_softmax_topk_rows(
        ...
        let mut args = RouterArgs {
            kind: ROUTER_SELECTED_SOFTMAX_TOPK,
            rows,
            columns: experts,
            k: top_k,
            route_scale: output_scale,
            logits: device_ptr(logits),
            indices: device_ptr(indices),
            weights: device_ptr(weights),
            ..Default::default()
    ...
    pub unsafe fn moe_gather_bf16_rows(
        ...
        source_rows: u32,
        route_rows: &DeviceBuffer<i32>,
        gathered: &mut DeviceBuffer<f32>,
        routes: u32,
        width: u32,
    ) -> KernelResult<()> {
        unsafe {
            self.moe(
                stream,
                MoeArgs {
                    kind: MOE_GATHER_BF16_ROWS,
                    batch_columns: routes,
                    tokens: source_rows,
                    hidden: width,
                    input: device_ptr(source),
                    route_slots: device_ptr(route_rows),
                    hidden_values: device_ptr(gathered),
                    ..Default::default()
                },
            )
        }
    ...
    pub unsafe fn moe_weighted_scatter_add_bf16_rows(
        ...
        unsafe {
            self.moe(
                stream,
                MoeArgs {
                    kind: MOE_WEIGHTED_SCATTER_ADD_BF16_ROWS,
                    batch_columns: routes,
                    tokens: output_rows,
                    hidden: width,
                    input: device_ptr(route_values),
                    route_weights: device_ptr(route_weights),
                    route_slots: device_ptr(route_rows),
                    output: device_ptr(output),
                    ..Default::default()
    ...
    pub(crate) unsafe fn transformer(
        ...
        invoke!(
            stream,
            args,
            ferrule_core_transformer_launch,
            "paged BF16 transformer operator"
        )
    }
...
mod tests {
    ...
    fn core_pod_layouts_match_native_abi() {
        ...
        assert_pod_layout!(RowsArgs, 88, kind, input_f32 => 16, 80);
        ...
        assert_eq!(offset_of!(TransformerArgs, key_cache_bf16), 144);
