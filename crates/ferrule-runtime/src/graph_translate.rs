...
use crate::dialects::{tensor_ops, transformer_ops};
...
use crate::transformer_plan::{TransformerLayerPlan, TransformerRuntimePlan};
...
pub fn build_semantic_transformer_graph_program_with_options(
    ...
    if inventory
        .hyper_connection_tensors(&plan.family)
        .iter()
...
fn dense_tensor_map(
    ...
    for tensor in inventory.dense_layer_tensors(&plan.family) {
        let key = (
