...
//! model/session scheduler and the device-independent runtime graph IR. They
...
//! ## Migration boundary
//!
//! These contracts are intentionally additive. Existing concrete model paths should
//! continue working unchanged. The expected migration path is:
//!
...
use ferrule_model::{TensorRole, TokenLogit};
...
/// Logits returned for one execution row.
...
/// Output for one row in an `ExecutionBatch`.
#[derive(Debug, Clone, PartialEq)]
pub struct ExecutionRowOutput {
    pub session_id: SessionId,
    pub position: usize,
    pub kv_handle: Option<KvHandle>,
    pub logits: RowLogits,
}

impl ExecutionRowOutput {
    pub fn new(
        session_id: SessionId,
        position: usize,
        kv_handle: Option<KvHandle>,
    ...
    pub fn no_logits(row: &ExecutionRow) -> Self {
        Self::new(row.session_id, row.position, row.kv_handle, RowLogits::None)
    }
}

/// Output envelope paired with an `ExecutionBatch`.
#[derive(Debug, Clone, PartialEq)]
pub struct ExecutionOutput {
    rows: Vec<ExecutionRowOutput>,
}

impl ExecutionOutput {
    pub fn new(rows: Vec<ExecutionRowOutput>) -> Result<Self> {
        if rows.is_empty() {
            return Err(Error::Internal(
                "execution output must contain at least one row".into(),
            ));
        }
        Ok(Self { rows })
    }

    pub fn empty_like_no_logits(batch: &ExecutionBatch) -> Result<Self> {
        let rows = batch
            .rows()
            .iter()
            .map(ExecutionRowOutput::no_logits)
            .collect();
        Self::new(rows)
    }

    pub fn rows(&self) -> &[ExecutionRowOutput] {
        &self.rows
    }

    pub fn logits_rows(&self) -> impl Iterator<Item = (usize, &ExecutionRowOutput)> + '_ {
        self.rows
            .iter()
            .enumerate()
            .filter(|(_, row)| !row.logits.is_none())
    }
}
...
mod tests {
    ...
    #[test]
    fn execution_output_pairs_with_batch_rows() {
        let batch =
            ExecutionBatch::prefill_last_logits(SessionId(3), 8, &[10, 11], Some(KvHandle(5)))
                .unwrap();
        let output = ExecutionOutput::empty_like_no_logits(&batch).unwrap();
        assert_eq!(output.rows().len(), 2);
        assert_eq!(output.rows()[0].session_id, SessionId(3));
        assert_eq!(output.rows()[0].position, 8);
        assert_eq!(output.rows()[0].kv_handle, Some(KvHandle(5)));
        assert!(output.logits_rows().next().is_none());
        ...
        .unwrap();
        assert_eq!(output.logits_rows().count(), 1);
    }
