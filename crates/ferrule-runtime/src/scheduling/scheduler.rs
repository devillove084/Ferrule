...
use crate::graph::runtime::{ExecutionBatch, ExecutionRow, ExecutionSegment, LogitsSelection};
use crate::scheduling::session::{
    GenerateRequest, RequestId, SequenceFinishReason, SequenceState, SessionId,
};
...
impl SchedulerAction {
    pub fn execution_batch(&self) -> Result<Option<ExecutionBatch>> {
        match self {
            Self::PrefillChunk(action) => action.execution_batch().map(Some),
            Self::DecodeBatch(actions) => {
                if actions.is_empty() {
                    return Ok(None);
                }
                let rows = actions.iter().map(DecodeAction::execution_row).collect();
                ExecutionBatch::new(ExecutionSegment::Decode, rows).map(Some)
            }
            Self::Finish { .. } | Self::Cancel { .. } => Ok(None),
        }
    }
}
...
impl PrefillChunkAction {
    ...
    pub fn execution_batch(&self) -> Result<ExecutionBatch> {
        ExecutionBatch::from_tokens(
    ...
    pub fn commit(&self, sequence: &mut SequenceState) -> Result<()> {
        ...
        sequence.commit_prefill_tokens(self.tokens.len())
    }
}
...
impl DecodeAction {
    ...
    pub fn execution_row(&self) -> ExecutionRow {
        ExecutionRow::new(
            self.token_id,
            self.position,
            self.session_id,
...
mod tests {
    use super::*;
    use crate::cache::KvHandle;
    use crate::graph::runtime::{ExecutionSegment, LogitsSelection};
    use crate::sampling::sampler::SamplingConfig;
    use crate::scheduling::SequenceStatus;
    ...
    #[test]
    fn prefill_action_materializes_execution_batch_and_commits_sequence() {
        let mut seq = SequenceState::new(SessionId(5));
        seq.request_id = Some(RequestId(9));
        seq.bind_kv(KvHandle(2));
        seq.set_prompt(&[10, 11, 12]);

        let SchedulerAction::PrefillChunk(first) = plan_prefill_chunk(&seq, 2).unwrap().unwrap()
        else {
            panic!("expected prefill action");
        };
        assert_eq!(first.request_id, Some(RequestId(9)));
        assert_eq!(first.token_range, 0..2);
        assert_eq!(first.tokens, vec![10, 11]);
        ...
        assert_eq!(batch.logits_rows().count(), 0);
        assert_eq!(batch.rows()[0].kv_handle, Some(KvHandle(2)));

        first.commit(&mut seq).unwrap();
        assert_eq!(seq.prompt_cursor, 2);
        assert_eq!(seq.position, 2);

        let SchedulerAction::PrefillChunk(last) = plan_prefill_chunk(&seq, 4).unwrap().unwrap()
        else {
        ...
        let batch = last.execution_batch().unwrap();
        assert_eq!(batch.positions().collect::<Vec<_>>(), vec![2]);
        assert_eq!(
            batch.logits_rows().map(|(i, _)| i).collect::<Vec<_>>(),
            vec![0]
        );

        last.commit(&mut seq).unwrap();
        assert!(seq.prompt_prefill_done());
        assert_eq!(seq.position, 3);
        assert!(plan_prefill_chunk(&seq, 1).unwrap().is_none());
    }

    #[test]
    fn decode_action_materializes_decode_execution_batch() {
        let mut seq = SequenceState::new(SessionId(6));
        seq.request_id = Some(RequestId(12));
        seq.bind_kv(KvHandle(4));
        seq.max_new_tokens = 2;
        seq.set_prompt(&[1, 2, 3]);
        seq.commit_prefill_tokens(3).unwrap();

        let action = DecodeAction::from_sequence(&seq, 42);
        assert_eq!(action.request_id, Some(RequestId(12)));
        assert_eq!(action.position, 3);

        let batch = SchedulerAction::DecodeBatch(vec![action])
            .execution_batch()
            .unwrap()
            .unwrap();
    ...
    #[test]
    fn terminal_scheduler_actions_do_not_materialize_batches() {
        let finish = SchedulerAction::Finish {
            request_id: Some(RequestId(1)),
            session_id: SessionId(1),
            reason: SequenceFinishReason::Eos,
        };
        assert!(finish.execution_batch().unwrap().is_none());

        let cancel = SchedulerAction::Cancel {
            request_id: Some(RequestId(2)),
            session_id: SessionId(2),
        };
        assert!(cancel.execution_batch().unwrap().is_none());
        assert!(SchedulerAction::DecodeBatch(Vec::new())
            .execution_batch()
            .unwrap()
            .is_none());
    }
...
mod batched_tests {
    ...
    fn step_returns_finished() {
        ...
        assert!(finished
            .iter()
            .all(|seq| seq.finish_reason == Some(SequenceFinishReason::MaxTokens)));
    ...
    fn preempt_least_generated_frees_blocks() {
        ...
        assert_eq!(bs.preempted()[0].status, SequenceStatus::Cancelled);
        assert_eq!(
            bs.preempted()[0].finish_reason,
