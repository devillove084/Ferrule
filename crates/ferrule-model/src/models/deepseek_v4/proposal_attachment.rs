//! DeepSeek-V4 Proposal attachment stored under the checkpoint's `mtp.*` namespace.
//!
...
use super::config::{
    DeepSeekV4AttentionConfig, ProposalAttachmentConfig,
...
/// remain a separate Proposal backbone input.
pub struct DeepSeekV4ProposalStage {
    /// Checkpoint stage index under `mtp.{stage}`.
    pub mtp_index: usize,
    /// Runtime layer identity. Proposal stages follow the base transformer layers
    /// so prepared-resource keys never alias `layers.0/1/2`.
    pub execution_layer: usize,
    /// The normal HC + attention + MoE body reused from the target model.
    pub transformer: DeepSeekV4Layer,
    /// Proposal's target-hidden projection exists only on stage zero.
    pub main_proj: Option<LinearWeight>,
    /// Normalization paired with `main_proj`; also stage-zero only.
    pub main_norm: Option<Vec<f32>>,
    /// Immutable routed-expert source catalog for this Proposal stage.
    pub expert_source_catalog: Arc<ExpertSourceCatalog>,
}

/// The last Proposal stage's prediction heads.
pub struct DeepSeekV4ProposalHeads {
...
/// Complete Proposal attachment with all checkpoint `mtp.*` stages.
pub struct DeepSeekV4ProposalAttachment {
    pub layers: Vec<DeepSeekV4ProposalStage>,
    pub prediction_heads: Option<DeepSeekV4ProposalHeads>,
    pub config: ProposalAttachmentConfig,
}

/// Frozen checkpoint-native Proposal row and commit contract.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DeepSeekV4ProposalProtocol {
...
impl TryFrom<&ProposalAttachmentConfig> for DeepSeekV4ProposalProtocol {
    type Error = Error;

    fn try_from(config: &ProposalAttachmentConfig) -> Result<Self> {
        if config.block_size == 0 {
            return Err(Error::Model {
                message: "DeepSeek-V4 Proposal gamma must be greater than zero".into(),
            });
        }
        let noise_token_id = config.noise_token_id.ok_or_else(|| Error::Model {
            message: "DeepSeek-V4 Proposal protocol requires a noise token id".into(),
        })?;
        if config.target_layer_ids.is_empty() {
            return Err(Error::Model {
                message: "DeepSeek-V4 Proposal protocol requires target hidden-state layers".into(),
            });
        }
        if config
            .target_layer_ids
        ...
        let target_verify_rows = config
            ...
            .ok_or_else(|| Error::Model {
                message: "DeepSeek-V4 Proposal target width overflow".into(),
            })?;
        Ok(Self {
...
impl DeepSeekV4ProposalProtocol {
    ...
    pub fn target_rows_for_drafts(&self, proposed_draft_tokens: usize) -> Result<usize> {
        if proposed_draft_tokens > self.gamma {
            return Err(Error::Model {
                message: format!(
                    "DeepSeek-V4 Proposal requested {proposed_draft_tokens} drafts above checkpoint gamma {}",
                    self.gamma
                ),
            });
        }
...
pub fn parse_proposal_hyper_connection_tensor(
    info: &HfSafetensorsTensorInfo,
...
pub fn load_proposal_stage(
    ...
    route_scale: f32,
) -> Result<DeepSeekV4ProposalStage> {
    let prefix = format!("mtp.{proposal_stage}.");
    ...
    let (main_proj, main_norm) = if proposal_stage == 0 {
        (
    ...
    let router_tensors: Vec<HfRouterTensorInfo> = layer_tensors
        ...
        .collect();
    let router = bind_router_from_hf(model_dir, proposal_stage, &router_tensors, reader)?;
    ...
    Ok(DeepSeekV4ProposalStage {
        mtp_index: proposal_stage,
        execution_layer,
...
pub fn load_proposal_heads(
    ...
    hc_config: HyperConnectionConfig,
) -> Result<DeepSeekV4ProposalHeads> {
    let prefix = format!("mtp.{proposal_stage}.");
...
impl DeepSeekV4ProposalAttachment {
    ...
    pub fn stage_zero_main_reference(&self, target_taps: &[f32], rows: usize) -> Result<Vec<f32>> {
        let stage_zero = self.layers.first().ok_or_else(|| Error::Model {
            message: "DeepSeek-V4 Proposal stage zero is missing".into(),
        })?;
        let projection = stage_zero.main_proj.as_ref().ok_or_else(|| Error::Model {
            message: "DeepSeek-V4 Proposal stage-zero main projection is missing".into(),
        })?;
        let norm = stage_zero
            .main_norm
            .as_deref()
            .ok_or_else(|| Error::Model {
                message: "DeepSeek-V4 Proposal stage-zero main norm is missing".into(),
            })?;
        ...
        let expected = rows.checked_mul(input_size).ok_or_else(|| Error::Model {
            message: "DeepSeek-V4 Proposal stage-zero reference input size overflow".into(),
        })?;
        if rows == 0 || target_taps.len() != expected || norm.len() != output_size {
            return Err(Error::Model {
                message: format!(
                    "DeepSeek-V4 Proposal stage-zero reference shape mismatch: rows={rows} taps={}/{} norm={}/{}",
                    target_taps.len(),
                    expected,
                    norm.len(),
                    output_size
...
mod tests {
    ...
    fn parses_mtp_hyper_connection_tensor_names() {
        ...
        let parsed = parse_proposal_hyper_connection_tensor(&info).unwrap();
    ...
    #[test]
    fn proposal_protocol_freezes_native_block_and_target_layout() {
        let model = DeepSeekV4ProposalAttachment {
            layers: Vec::new(),
            prediction_heads: None,
            config: ProposalAttachmentConfig {
                block_size: 5,
