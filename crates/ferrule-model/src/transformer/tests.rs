...
fn prepared_executable_declares_real_decoder_stages_and_manifests() {
    ...
    assert_eq!(
        ...
        &crate::execution::TransformerStage::Embed
    );
    assert_eq!(
        executable.stages()[1].operation(),
        &crate::execution::TransformerStage::Attention { layer: 0 }
    );
    assert_eq!(
        executable.stages()[2].operation(),
        &crate::execution::TransformerStage::Router { layer: 0 }
    );
    assert_eq!(
        executable.stages()[3].operation(),
        &crate::execution::TransformerStage::FeedForward { layer: 0 }
    );
    assert_eq!(
        executable.stages().last().unwrap().operation(),
        &crate::execution::TransformerStage::Output
    );
    assert!(
        executable
            .stages()
            ...
            .filter(|stage| !matches!(
                stage.operation(),
                crate::execution::TransformerStage::FeedForward { .. }
            ))
