//! Quantized weight pack with versioned manifest.
//!
//! Format v2:
//!   magic "FERRULEW" 8B | manifest_len u64 LE | manifest JSON | num_layers u64 LE | layers...
//! Format v1: manifest_len == 0
...
const WEIGHTPACK_MAGIC: &[u8; 8] = b"FERRULEW";
const CURRENT_FORMAT_VERSION: u32 = 2;

// ── Manifest ────────────────────────────────────────────────────────────

#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct WeightPackManifest {
    ...
    #[serde(default)]
    pub tensor_shapes: Vec<WeightPackTensorInfo>,
    pub quant_suffix: String,
}

#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq)]
pub struct WeightPackTensorInfo {
    pub name: String,
    pub shape: Vec<usize>,
}

impl WeightPackManifest {
    pub fn new(
        quant_type: &str,
        layout_version: &str,
        num_layers: usize,
...
pub fn weightpack_path(model_dir: &Path, qt_suffix: &str) -> std::path::PathBuf {
    model_dir.join(format!("model.{qt_suffix}.weightpack"))
}

/// Stable, cheap fingerprint for weight-pack compatibility checks.
///
/// This intentionally hashes `config.json` contents plus safetensors file names
/// and sizes. It catches common stale-artifact cases without forcing a full
/// model-weight read before the weight-pack-only path.
...
pub fn read_u64(data: &[u8], pos: &mut usize) -> Result<u64> {
    let end = pos
        .checked_add(8)
        .ok_or_else(|| Error::Internal("weight pack offset overflow".into()))?;
    let bytes = data
        .get(*pos..end)
        .ok_or_else(|| Error::Internal("weight pack u64 truncated".into()))?;
...
pub fn read_buf_slice<'a>(data: &'a [u8], pos: &mut usize) -> Result<&'a [u8]> {
    ...
    let end = pos
        .checked_add(len)
        .ok_or_else(|| Error::Internal("weight pack buffer offset overflow".into()))?;
    let slice = data
        .get(*pos..end)
        .ok_or_else(|| Error::Internal("weight pack buffer truncated".into()))?;
    *pos = end;
    Ok(slice)
}
...
pub fn bytes_to_f32_vec(data: &[u8]) -> Result<Vec<f32>> {
    ...
    if !chunks.remainder().is_empty() {
        return Err(Error::Internal(
            "weight pack f32 buffer has non-multiple-of-4 length".into(),
        ));
    }
    Ok(out)
}
...
pub struct WeightPackLayerData {
...
pub struct WeightPackReader {
    mmap: memmap2::Mmap,
    manifest: Option<WeightPackManifest>,
    num_layers: usize,
    layer_offsets: Vec<(usize, usize)>,
}

impl WeightPackReader {
    pub fn open(path: &Path) -> Result<Self> {
        let file = std::fs::File::open(path)
            .map_err(|e| Error::Internal(format!("weight pack open: {e}")))?;
        let mmap = unsafe {
            memmap2::MmapOptions::new()
                .map(&file)
                .map_err(|e| Error::Internal(format!("weight pack mmap: {e}")))?
        };
        if mmap.len() < 8 || &mmap[..8] != WEIGHTPACK_MAGIC {
            return Err(Error::Internal("invalid weight pack magic".into()));
        }
        let mut pos = 8usize;
    ...
    fn parse_header(
        data: &[u8],
        pos: &mut usize,
    ) -> Result<(Option<WeightPackManifest>, usize, Vec<(usize, usize)>)> {
        let manifest_len = read_u64(data, pos)? as usize;
        let manifest = if manifest_len > 0 {
            let end = pos
                .checked_add(manifest_len)
                .ok_or_else(|| Error::Internal("weight pack manifest offset overflow".into()))?;
            if end > data.len() {
                return Err(Error::Internal("weight pack manifest truncated".into()));
            }
            let json = &data[*pos..end];
            *pos = end;
            let m: WeightPackManifest = serde_json::from_slice(json)
                .map_err(|e| Error::Internal(format!("weight pack manifest json: {e}")))?;
            if m.format_version > CURRENT_FORMAT_VERSION {
                return Err(Error::Internal(format!(
                    "weight pack format v{} > supported v{}",
        ...
        for _ in 0..num_layers {
            ...
            let end = pos
                ...
                .ok_or_else(|| Error::Internal("weight pack layer offset overflow".into()))?;
            if end > data.len() {
                return Err(Error::Internal("weight pack layer truncated".into()));
            }
    ...
    pub fn manifest(&self) -> Option<&WeightPackManifest> {
    ...
    pub fn layer(&self, li: usize) -> Result<WeightPackSlice<'_>> {
        let (start, end) = *self
            .layer_offsets
            .get(li)
            .ok_or_else(|| Error::Internal(format!("weight pack layer {li} out of range")))?;
...
pub fn write_weightpack(
    path: &Path,
    layers: &[WeightPackLayerData],
    manifest: Option<&WeightPackManifest>,
) -> Result<()> {
    use std::io::Write;
    let tmp = path.with_extension("weightpack.tmp");
    let mut f = std::io::BufWriter::new(
        std::fs::File::create(&tmp)
            .map_err(|e| Error::Internal(format!("weight pack create: {e}")))?,
    );
    f.write_all(WEIGHTPACK_MAGIC)
        .map_err(|e| Error::Internal(format!("weight pack write: {e}")))?;
    ...
    f.into_inner()
        .map_err(|e| Error::Internal(format!("weight pack flush: {e}")))?;
    std::fs::rename(&tmp, path).map_err(|e| Error::Internal(format!("weight pack rename: {e}")))?;
    Ok(())
}

// ── Streaming weight-pack writer ────────────────────────────────────────

/// Write a weight pack incrementally — appends one layer at a time.
///
...
pub struct StreamingWeightPackWriter {
    file: std::fs::File,
...
impl StreamingWeightPackWriter {
    /// Create a new streaming weight-pack file and write the header.
    ///
    /// The layer count is written as a placeholder (0) and will be
    /// updated when [`finish`](Self::finish) is called.
    pub fn create(path: &Path, manifest: Option<&WeightPackManifest>) -> Result<Self> {
    ...
    /// Append a single quantized layer to the weight pack.
    pub fn append_layer(&mut self, data: &WeightPackLayerData) -> Result<()> {
        use std::io::Write;
    ...
    /// Finalize the weight pack: write the actual layer count and flush.
    pub fn finish(mut self) -> Result<()> {
        ...
        self.file
            .set_len(end)
            .map_err(|e| Error::Internal(format!("weight pack truncate: {e}")))?;
        Ok(())
    }
...
mod tests {
    ...
    fn empty_layer() -> WeightPackLayerData {
        WeightPackLayerData {
            qp_packed: vec![1, 2, 3],
            qp_scales: vec![10, 20],
            kp_packed: vec![],
    ...
    fn manifest_serialize_roundtrip() {
        let m = WeightPackManifest::new("Q4_0", "q4_0_llama", 16, "abc123", "q4_0_llama");
        let json = serde_json::to_string(&m).unwrap();
        let parsed: WeightPackManifest = serde_json::from_str(&json).unwrap();
    ...
    fn write_read_v2_with_manifest() {
        let tmp = std::env::temp_dir().join("ferrule_test_v2.weightpack");
        let m = WeightPackManifest::new("Q4_0", "q4_0_llama", 1, "testhash", "q4_0_llama");
        write_weightpack(&tmp, &[empty_layer()], Some(&m)).unwrap();
        let reader = WeightPackReader::open(&tmp).unwrap();
    ...
    fn read_v1_no_manifest() {
        let tmp = std::env::temp_dir().join("ferrule_test_v1.weightpack");
        let mut buf = Vec::new();
        buf.extend_from_slice(b"FERRULEW");
    ...
    fn reject_future_format() {
        let tmp = std::env::temp_dir().join("ferrule_test_future.weightpack");
        let mut buf = Vec::new();
        buf.extend_from_slice(b"FERRULEW");
        ...
        std::fs::write(&tmp, &buf).unwrap();
        assert!(WeightPackReader::open(&tmp).is_err());
        let _ = std::fs::remove_file(&tmp);
    }

    #[test]
    fn weightpack_path_naming() {
        let p = weightpack_path(Path::new("/tmp/model"), "q4_0_llama");
        assert!(p.to_string_lossy().contains("model.q4_0_llama.weightpack"));
    }

    #[test]
    fn streaming_writer_empty_file_readable() {
        let tmp = std::env::temp_dir().join("ferrule_test_streaming_empty.weightpack");
        let writer = StreamingWeightPackWriter::create(&tmp, None).unwrap();
    ...
    fn streaming_writer_one_layer_with_manifest() {
        let tmp = std::env::temp_dir().join("ferrule_test_streaming_one.weightpack");
        let m = WeightPackManifest::new("Q4_0", "q4_0_llama", 1, "hash1", "q4_0_llama");
        let mut writer = StreamingWeightPackWriter::create(&tmp, Some(&m)).unwrap();
    ...
    fn streaming_writer_multiple_layers() {
        let tmp = std::env::temp_dir().join("ferrule_test_streaming_multi.weightpack");
        let mut writer = StreamingWeightPackWriter::create(&tmp, None).unwrap();
        for _ in 0..3 {
            writer.append_layer(&empty_layer()).unwrap();
        }
