...
impl CudaTransformerExecutor<'_> {
    ...
    pub(crate) fn attention_step(&mut self, li: usize) -> Result<()> {
        ...
        // Attention norm.
        cu(m.rms_norm_fused(s, cfg1(d), hidden, &layer.an, normed, d as u32, eps))?;
        ...
        cu(m.rms_norm_fused(s, cfg1(d), q, &layer.qn, q_tmp, d as u32, eps))?;
        cu(m.rms_norm_fused(s, cfg1(kv_dim), k, &layer.kn, k_tmp, kv_dim as u32, eps))?;
        ...
        cu(m.add(s, cfg(d), hidden, q, h_tmp1, d as u32))?;
